...
def Force(data, F):
    ...
    data.ρii[:] += data.qF[1: data.nmols+1] ** 2
    data.ρii[:] += data.pF[1: data.nmols+1] ** 2
    data.ρii[:] += data.qB[1: data.nmols+1] ** 2
    data.ρii[:] += data.pB[1: data.nmols+1] ** 2
    
    F[:]        = F[:] - data.ωj_nr_sq * data.R
    F[:]        = F[:] - 0.25 * np.kron(np.ascontiguousarray(data.ρii), param.cj)
...
def Umap(data):
    ...
    data.qF[:], data.pF[:] = (np.real(data.ψFΔ[:]) * np.sqrt(2)), (-np.imag(data.ψFΔ[:]) * np.sqrt(2))
    data.qB[:], data.pB[:] = (np.real(data.ψBΔ[:]) * np.sqrt(2)), (-np.imag(data.ψBΔ[:]) * np.sqrt(2))
...
def VelVer(data):
    ...
    Force(data, data.F1)
    data.R[:]  += data.v[:] * param.dtN + 0.5 * data.F1 * param.dtN ** 2 / param.M
    Force(data, data.F2)
    data.v[:]  += 0.5 * (data.F1[:] + data.F2[:]) * param.dtN / param.M
